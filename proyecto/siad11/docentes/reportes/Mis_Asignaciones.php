...
class PDF extends FPDF
{
	...
	public $NombreDocente;
		function Header()
		{
		    ...
		    $codigo = $_SESSION["Codigo"];
		        $docente = mysqli_query ($this->conexion,"SELECT concat(NombresDocente, ' ',ApellidosDocente) as Docente from docentes where idDocente = '$codigo'");
		            while($row = mysqli_fetch_row($docente)){
		            $this->NombreDocente = $row[0];

            }

		 $this->Cell(75,20, $this->NombreDocente, 0,0,'R');
			$this->Ln(15);
		...
		while ($asignaciones2 = mysqli_fetch_array($asignaciones)) {
			...
			$pdf->Cell(35, 8, mb_convert_encoding($asignaciones2['Asignacion'], 'ISO-8859-1', 'UTF-8'), 0, 'C');
			$pdf->Cell(20, 8, $asignaciones2['NumeroA'], 0, 'C');
			$pdf->Cell(40, 8, mb_convert_encoding($asignaciones2['Asignatura'], 'ISO-8859-1', 'UTF-8'), 0, 'C');
