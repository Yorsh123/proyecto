...
class PDF extends FPDF
{
	...
	public $NombreDocente;
		function Header()
		{
		    ...
		    $docente = $_POST['docente'];
            $asignaciones = mysqli_query($this->conexion,"SELECT concat(NombresDocente, ' ' ,ApellidosDocente) as Docente FROM docentes where idDocente = '$docente'");
       ...
       where docentes.idDocente = '$docente' ");
        ...
        $item = 0;
			while($asignaciones2 = mysqli_fetch_array($asignaciones)){
				...
				$pdf->Cell(30, 8, ($asignaciones2['Asignacion']), 0,'C');
				$pdf->Cell(20, 8, $asignaciones2['NumeroA'], 0, 'C');
				$pdf->Cell(40, 8, ($asignaciones2['Asignatura']), 0,'C');
